...
report 50064 "Commande cadre achat"
{
    DefaultLayout = RDLC;
    RDLCLayout = './src/ReportLayout/CommandeCadreAchat.rdlc';
    Caption = 'Commande cadre achat';
        ...
        dataitem("Purchase Header"; "Purchase Header")
        {
            ...
            dataitem(CopyLoop; "Integer")
            {
                ...
                dataitem(PageLoop; "Integer")
                {
                    ...
                    column(MentionCadreTxt;MentionCadreTxt)
                    {
                    }
                    column(DateValiditeCaption; DateValiditeLbl)
                    {
                    }
                    column(DateValidite; "Purchase Header"."Date validite")
                    {
                    }
                    column(LabelCommentaire; LabelCommentaireLbl)
                    {
                    }
                    ...
                    dataitem(RoundLoop; "Integer")
                    {
                        ...
                        column(dTotalDEEE; dTotalDEEE)
                        {
                        }
                        column(TotalDEEEText; TotalDEEEText)
                        {
                        }
                        column(VendorItemNo_PurchLine; "Purchase Line"."Vendor Item No.")
                        {
                        }
                        column(VendorItemNoCaption; "Purchase Line".FieldCaption("Vendor Item No."))
                        {
                        }
                        column(ItemGencod; Article."No. client final")
                        {
                        }
                        column(ItemGencodCaption; Article.FieldCaption("No. client final"))
                        {
                        }
                        column(HSCode_PurchLine; HSCode_wTxt)
                        {
                        }
                        ...
                        column(QteRestanteCdeCadre; QteRestanteCdeCadre)
                        {
                            DecimalPlaces = 0:5;
                        }
                        ...
                        //column(No_PurchLineCaption; ReferenceLbl)
                        //{
                        //}
                        column(LabelRef; ReferenceLbl)
                        {
                        }
                        ...
                        begin
                            ...
                            if (TempPurchaseLine.Type = TempPurchaseLine.Type::"G/L Account") then
                                "Purchase Line"."No." := '';
                            ...
                            QteRestanteCdeCadre := "Purchase Line".CalcQteDispoCdeCadre();
                            //KAN.FHA 17/03/2026 FIN
                            //KAN.FHA 11/03/2026 DEBUT
                            Article.Init();
                            if "Purchase Line".Type = "Purchase Line".Type::Item then
                                if Article.Get("Purchase Line"."No.") then;

                            Clear(HSCode_wTxt);
                            if ("Purchase Line".Type = "Purchase Line".Type::Item) and ("Purchase Line"."No." <> '') and (Article."Tariff No." <> '') then
                                HSCode_wTxt := StrSubstNo(LabelHSCode, Article."Tariff No.");
            ...
            begin
                ...
                CurrReport.FormatRegion := LanguageCodeunit.GetFormatRegionOrDefault("Format Region");
                ...
                if "Language Code" in ['','FR','FRA'] then
                    MentionCadreTxt := LabelMentionCadreFRA
                else
                    MentionCadreTxt := LabelMentionCadreENU;
    ...
    begin
        GLSetup.Get();
        CompanyInfo.Get();
        //KAN.FHA 12/03/2026 DEBUT
        CompanyInfo.CalcFields(Picture);
        //KAN.FHA 12/03/2026 FIN
    ...
    var
        ...
        DateValiditeLbl: Label 'Valid until : ';
        LabelCommentaireLbl: Label 'Comments :';
        BlanketPurchOrderNoCaptionLbl: Label 'Bulk Order No.';
        CompanyInfoRegNoCaptionLbl: Label 'SIRET No.';
        VersionLbl: Label 'Version';
        LabelHSCode: Label 'HS Code : %1';
        ReferenceLbl: Label 'Reference';
        LabelMentionCadreFRA: Label 'La présente commande d’engagement fixe les conditions tarifaires et commerciales applicables pendant la période de validité indiquée. Les quantités mentionnées sont prévisionnelles et n''engagent pas l''acheteur. Les livraisons seront déclenchées par des appels de commande émis ultérieurement';
 
        LabelMentionCadreENU: Label 'This blanket purchase order sets out the pricing and commercial terms applicable during the stated validity period. The quantities indicated are estimates only and shall not be binding on the Buyer. Deliveries will be triggered by purchase orders issued subsequently.';
